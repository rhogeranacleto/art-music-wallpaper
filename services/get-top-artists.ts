...
export async function getTopArtists(
  ...
  if (limit) {
    ...
    const { data } = await axios.get<ITopArtists>(
      `${base_last_fn_api}/?method=user.gettopartists&user=${name}&api_key=${api_key}&format=${format}&limit=${limit}&period=${period}`,
    );
...
async function getImageFromLink(imgLinkElements: Cheerio, count = 1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const { data } = await axios.get(
      `https://www.last.fm${imgLinkElement.attr('href')}`,
    );
